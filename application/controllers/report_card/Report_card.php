...
class Report_card extends MY_controller{
	...
	public function index(){
		...
		$this->render_template('report_card/report_card_term',$data);
	}
	...
	public function make_report_card_int(){
		...
		$school_setting = $this->alam->select('school_setting','*');
		$stu_data = $this->alam->report_card_student_detail_int($trm,$classs,$sec);
	...
	public function make_report_card_annual_XI(){
		...
		$school_setting = $this->alam->select('school_setting','*');
		$stu_data = $this->alam->report_card_student_detail_xi($trm,$classs,$sec);
...
public function generatePDF_VI_VIII_T1_new()
	{
		...
		$result = array();
		foreach ($adm_no as $key => $val) {
			$getStu = $this->alam->selectA('student', 'ADM_NO,ROLL_NO,FIRST_NM,FATHER_NM,MOTHER_NM,DISP_CLASS,DISP_SEC,C_MOBILE,BIRTH_DT', "CLASS='$classs' AND SEC='$sec' AND Student_Status='ACTIVE' AND ADM_NO='$val'");
	...
	public function generatePDF()
	{
		...
		if($examode == 1)//diffrentiate CBSE or CMC
		{
			if($term == 1)
			{
				...
				{
					$term = 'TERM-1';
					$examcode = array('1','4','19');
				}
				elseif($classs == 5 || $classs == 6 || $classs == 7)
				{
					$term = 'TERM-1';
				...
				foreach ($examcode as $keys => $val) {
							...
							if($val == 1){
								...
								if($pt_type == 1)
								{
									...
									$total_ab_count['AB'] = (!isset($total_ab_count['AB']))?0:$total_ab_count['AB'];
									$ab = ($absent_count == $total_ab_count['AB'])?'AB':'0';
									$final_marks[$keys] = ($ab == 'AB')?$ab:max($mark);
								...
								if(!empty($all_marks))
								{
									if($val == 12)
									{
										...
										if($classs=="5" || $classs=="6" || $classs=="7")
										{
										$mark = ($all_marks[0]['M3']/$all_marks[0]['M1']) * 10 ;
										$mark = ($all_marks[0]['M2'] == 'AB' || $all_marks[0]['M2'] == '-')?$all_marks[0]['M2']:$mark;
										}
										else
										{
									...
									if($val == 4)
									{
										...
										if($classs=="5" || $classs=="6" || $classs=="7")
										{
											...
											elseif($sub_code =='8' || $sub_code =='10' || $sub_code =='18' || $sub_code =='16' || $sub_code =='22' || $sub_code =='35' || $sub_code =='36')
											{
												...
												//echo '<br/>';
												$mark = ($all_marks[0]['M3']/$all_marks[0]['M1']) * 70;
												$mark = ($all_marks[0]['M2'] == 'AB' || $all_marks[0]['M2'] == '-')?$all_marks[0]['M2']:$mark;
											}
											else
											{
								...
								{
									$final_marks[$keys] = ($round_off==1)?round($mark): $mark;								
								}
					...
					$skill=$this->db->query("SELECT * FROM `marks_all` WHERE admno='$admnum' AND EXAMCODE='4' AND SUBJECT='$sub_code'")->result_array();
						...
						$rt_markss = ($marks['REVISION-TEST-I'] == 'AB' || $marks['REVISION-TEST-I'] == '-') ? "N/A" : $marks['REVISION-TEST-I'];
							if ($class == '1' || $class == '2' || $class == '3' || $class == '4' || $class == 5 || $class == 6 || $class == 7) {
								$rt_marks = $rt_markss;
							}else{
								$rt_marks = $rt_markss /2;
							}
						...
						if($class == 5 || $class == 6 || $class == 7){
							$marks['marks_obtained'] = round($marks_obtained,0);
						}else{
							$marks['marks_obtained'] = ($round_off==1)?round($marks_obtained): number_format($marks_obtained,2);
						}
						...
						if ($class == 1 || $class == 2) {
							...
							$result[$value]['sub'][$key2]['subject_name'] = $val2['subj_nm'];
							$skill = $this->db->query("SELECT * FROM `marks_all` WHERE admno='$admnum' AND EXAMCODE='4' AND SUBJECT='$sub_code'")->result_array();
							$marks['skill1'] = $skill[0]['m3'];
							$marks['skill2'] = $skill[1]['m3'];
							$result[$value]['sub'][$key2]['marks'] = $marks;
						} else {
							$sub_code = $val2['subject_code'];
							$result[$value]['sub'][$sub_code]['subject_name'] = $val2['subj_nm'];
							foreach ($examcode as $keys => $val) {
								$all_marks = $this->sumit->fetchAllData('M1,M2,M3,ExamC', 'marks', "admno='$value' AND ExamC = $val AND SCode='$sub_code' AND Term='$term' AND status = '1'");
								$marks = $all_marks[0]['M2'] == 'AB' ? 'AB': $all_marks[0]['M3'];
								if($val == 1){
									$result[$value]['sub'][$sub_code]['marks']['pt'] = $marks;
								}elseif($val == 4){
									$result[$value]['sub'][$sub_code]['marks']['hy'] = $marks;
								}elseif($val == 19){
									$result[$value]['sub'][$sub_code]['marks']['ft'] = $marks;
								}
								

							}
							// $marks[$sub_code] = $all_marks[0]['m3'];
							// $result[$value]['sub'][$key2]['marks'] = $marks;
						}
					}
				}
			...
			if($class == '1' || $class == '2' || $class == '3' || $class == '4'||$class == '5' || $class == '6' || $class == '7'){
				...
				if($class == '5' || $class == '6' || $class == '7'){
					...
					//echo '<pre>';print_r($data);die;
					$this->load->view('report_card/report_card_cbsc_pdf_NUR_PREP_NEW', $data);
					//$this->load->view('report_card/report_card_cbsc_pdf_PREP_II',$data);
					//
				}
				elseif( $class == '3' || $class == '4'){
					$this->load->view('report_card/report_card_cbsc_pdf_I_II_NEW',$data);
	...
	public function generatePDF_annual_XI_theory()
	{
		$school_setting = $this->alam->select('school_setting', '*');
		$school_photo   = $this->alam->select('school_photo', '*');
		$reportCardType_data = $this->alam->select('misc_table', '*');
		...
		$pt_all_marks = array();
		$result = array();
		//diffrentiate CBSE or CMC
		
		if ($examode == 1) //diffrentiate CBSE or CMC
		{
			if ($term == 1) {
				$term = 'TERM-1';
				$examcode = array('4', '10', '11', '15', '18');
			} else {
				$term = 'TERM-2';
				$examcode = array('1', '2', '3', '5');
			}
			...
			foreach ($adm_no as $key => $value) {
				$stu_data = $this->alam->studentDetailsByAdmissionNo_xi($value, $termId);
				$result[$value] = $stu_data;
				$admnum  = $stu_data['ADM_NO'];
				$class   = $stu_data['CLASS'];
				$section = $stu_data['SEC'];
				...
				foreach ($subjectData as $key2 => $val2) {
					...
					if ($check_student_subject) {
						$sub_code = $val2['subject_code'];
						$subname = $val2['subj_nm'];
						$pt_type = $val2['pt_type'];
						$final_marks = array();
						$result[$value]['sub'][$key2]['subject_name'] = $val2['subj_nm'];
						$result[$value]['sub'][$key2]['opt_code'] = $val2['opt_code'];
						$result[$value]['sub'][$key2]['subject_code'] = $val2['subject_code'];

						foreach ($examcode as $keys => $val) {

							$examC = $val;
							if ($val == '4' || $val == '10' || $val == '11' || $val == '15' || $val == '18') {
								$Tm = 'TERM-1';
							} else {
								$Tm = 'TERM-2';
							}
							...
							$wetageMarks = $this->sumit->fetchSingleData('wetage1,wetage2', 'exammaster', array('ExamCode' => $val));
							// echo 'wet 1 of ExamCode  '.$val.' = '.$wetageMarks['wetage1'].'<br>';
							// echo 'wet 2 of ExamCode  '.$val.' = '.$wetageMarks['wetage2'].'<br>';
							$absent = array();
							
							$ab = 0;
							if ($sub_code == 24 || $sub_code == 6 || $sub_code == 33 || $sub_code == 38 || $sub_code == 4 || $sub_code == 105 || $sub_code == 42)
							{$wt1=60;}
							else{$wt1=70;}
							//THEORY key=3
							if ($val == 15) {
								...
								if ($pt_type == 1) {
									foreach ($all_marks as $key4 => $value4) {
										$mark[$key4] = ($value4['M3'] / $value4['M1']) * $wt1;
										$absent[$key4] = $value4['M2'];
									}
									$absent_count = count($absent);
									$total_ab_count = array_count_values($absent);
									$total_ab_count['AB'] = (!isset($total_ab_count['AB'])) ? 0 : $total_ab_count['AB'];
									$ab = ($absent_count == $total_ab_count['AB']) ? 'AB' : '0';
									$final_marks[$keys] = ($ab == 'AB') ? $ab : number_format(max($mark), 2); //key == 2 Theory
									$wetage_15 = $wetageMarks['wetage1'];
								}
								$final_marks[$keys] = ($round_off == 1) ? round($final_marks[$keys]) : $final_marks[$keys];
								// echo 'if =>' . $keys . '<br>';
							}
							...
							elseif ($val == 18) {
								...
								if (!empty($all_marks)) {
									...
									$mark = ($all_marks[0]['M3'] / $all_marks[0]['M1']) * $wt_prac;
									$mark = ($all_marks[0]['M2'] == 'AB' || $all_marks[0]['M2'] == '-') ? $all_marks[0]['M2'] : $mark;
									// echo $sub_code.$mark.'<br>';
								} else {
									$mark = 0;
								}
								if ($mark == 'AB' || $mark == '-') {
							...
							elseif ($val == 10) {
								...
								if ($mark == 'AB' || $mark == '-') {
									...
									$final_marks[$keys] = ($round_off == 1) ? ceil($mark) : number_format($mark, 2);
								}
								$wetage_10 = $wetageMarks['wetage1'];
						...
						$marks['notebook'] = 0;
						$marks['subject_enrichment'] = 0;
						$marks['half_yearly'] = $final_marks[0]; // Term-1
						$marks['unit1'] = $final_marks[1]; // Unit test 1
						$marks['unit2'] = $final_marks[2]; // Unit test 2
						$marks['theory'] = $final_marks[3]; // Theory
						$marks['practical'] = $final_marks[4]; // practical

						$unit1 = ($marks['unit1'] == 'AB' || $marks['unit1'] == '-') ? '0' : $marks['unit1'];
						$unit2 = ($marks['unit2'] == 'AB' || $marks['unit2'] == '-') ? '0' : $marks['unit2'];
						$ut_marks = ($unit1 > $unit2) ? $unit1 : $unit2;
						
						$hy_marks = ($marks['half_yearly'] == 'AB' || $marks['half_yearly'] == '-') ? '0' : $marks['half_yearly'];
						$theory_marks = ($marks['theory'] == 'AB' || $marks['theory'] == '-') ? '0' : $marks['theory'];
						$practical_marks = ($marks['practical'] == 'AB' || $marks['practical'] == '-') ? '0' : $marks['practical'];

						$marks['theory'] = $theory_marks + $ut_marks;
						$marks['obtained_th'] = $theory_marks + $practical_marks + $ut_marks;
						$marks_obtained_th =  $theory_marks + $practical_marks + $ut_marks;
						$marks['obtained'] = $hy_marks;
						$marks_obtained = $hy_marks;
						$marks['marks_obtained'] = ($round_off == 1) ? round($marks_obtained) : number_format($marks_obtained, 2);
						$gradeData = $this->sumit->fetchSingleData('Grade,Qualitative_Norms', 'grademaster', "ORange >=$marks_obtained AND CRange <= $marks_obtained");
						$marks['grade'] = $gradeData['Grade'];
						$result[$value]['sub'][$key2]['marks'] = $marks;
						...
						// echo implode(" ",$arr);
						if ($flag_th > 1) {
								if($admnum == '0446/2013' || $admnum == '0244/2010' || $admnum == '0595/2023' || $admnum == '0044/2010' || $admnum == '1183/2022' || $admnum == '0140/2010' || $admnum == '0032/2009'){
								$rem = $result[$value]['rsult'] = 'Detained + TC';
							}else{
								$rem = $result[$value]['rsult'] = 'Detained';
							}
						} elseif ($flag_pr >= 1) {
							if ($flag_th == 1) {
								
									$rem = $result[$value]['rsult'] = 'Compartment in ' . $failinth . ' theory and re-test in ' . implode(", ",$sub_pr).' practical';
								
							} else {
								$rem = $result[$value]['rsult'] = 'Re-test in ' . implode(", ",$sub_pr).' practical';
							}
						} elseif ($flag_th == 1) {
							$rem = $result[$value]['rsult'] = 'Compartment in ' . $failinth. ' theory';
						} else {
							$rem = $result[$value]['rsult'] = 'Pass';
						}
					}
				}
				$this->db->query("INSERT INTO remarks_xi (ADM_NO, REMARKS,CLASS,SEC) VALUES ('$admnum', '$rem','$classs','$sec')
				ON DUPLICATE KEY UPDATE ADM_NO = '$admnum'");
			}

			$data['wet_10'] = $wetage_10;
			$data['wet_15'] = $wetage_15;
			$data['wet_18'] = $wetage_18;
			$data['wet_4'] = $wetage_4;
			...
			$this->load->view('report_card/report_card_cbsc_pdf_annual_XI_theory', $data);
		}
...
function adpdf_int(){
	...
	function adpdfNur()
	{
	    ...
	    </head><body><div style='border:0px solid #000; padding:10px;'>";
		$html .= $this->input->post('value');
		$html .= "</div></body></html>";
	...
	public function make_report_card_tabulation($pdf=null,$trm=null,$classs=null,$sec=null,$date=null,$round=null){
		...
		if($examode == 1)//diffrentiate CBSE or CMC
		{
			...
			if($trm == 1){ 
				$trm = 'TERM-1';
				$examList = $this->alam->selectA('exammaster','*',"ExamCode in('1','12','4')");
			}else{
				$trm = 'TERM-2';
