...
class Teacher_master extends MY_Controller{
	...
	public function examination(){
		...
		if($examcode == 1 || $examcode == 2 || $examcode == 3 || $examcode == 6 || $examcode == 7 || $examcode == 8){
			if($classcode == '13'){
				  $sqldata = $this->db->query("SELECT DISTINCT(new_studentsubject_xi.SUBCODE) as subject_code,subjects.SubName as subj_nm  FROM `new_studentsubject_xi`
JOIN subjects ON subjects.SubCode=new_studentsubject_xi.SUBCODE
WHERE Class='$classcode'")->result();
			}else{
