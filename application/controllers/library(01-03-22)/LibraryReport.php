...
class LibraryReport extends MY_controller{
	...
	public function index(){		
		...
		{		
			...
			if($rpttyp==0){
				...
				$data['isuu_rpt']   = $this->pawan->selectA('books_applied','*'," Issued='1' and IDate BETWEEN '$fromdt' AND '$to_dt'");
			}elseif($rpttyp==1){
				$data['title']	='All Issued Report From '.$fromdt.' To '.$to_dt;
				$data['isuu_rpt']   = $this->pawan->selectA('books_applied','*'," IDate BETWEEN '$fromdt' AND '$to_dt'");
			}else{
				$data['title']	='Defaulter Report From '.$fromdt.' To '.$to_dt;
				$data['isuu_rpt']   = $this->pawan->selectA('books_applied','*',"RDate>Due_date AND IDate BETWEEN '$fromdt' AND '$to_dt'");
			}
		} 		
	...
	public function book_report_emp(){		
		...
		{		
			...
			if($rpttyp==0){
				$data['title']	=' Issued Report From '.$fromdt.' To '.$to_dt;
				$data['isuu_rpt']   = $this->pawan->selectA('books_applied1','*'," Issued='1' and IDate BETWEEN '$fromdt' AND '$to_dt'");
			}elseif($rpttyp==1){
				$data['title']	='All Issued Report From '.$fromdt.' To '.$to_dt;
				$data['isuu_rpt']   = $this->pawan->selectA('books_applied1','*'," IDate BETWEEN '$fromdt' AND '$to_dt'");
			}else{
				$data['title']	='Defaulter Report From '.$fromdt.' To '.$to_dt;
				$data['isuu_rpt']   = $this->pawan->selectA('books_applied1','*',"RDate>Due_date AND IDate BETWEEN '$fromdt' AND '$to_dt'");
			}
		} 		
