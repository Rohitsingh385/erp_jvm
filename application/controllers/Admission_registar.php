...
class Admission_registar extends MY_Controller{
	...
	public function student_details_date_wise(){
		...
		$session_master = $this->dbcon->select('session_master','*',"Active_Status=1");
		$year = $session_master[0]->Session_Year;
		$student = $this->dbcon->select('student st','ADM_NO,ROLL_NO,FIRST_NM,BIRTH_DT,FATHER_NM,MOTHER_NM,ADM_DATE,DISP_CLASS, DISP_SEC,(SELECT CLASS_NM FROM classes WHERE classes.Class_No=st.ADM_CLASS)ADM_CLASS_id,(SELECT stoppage FROM stoppage WHERE stoppage.STOPNO=st.STOPNO)other_stop',"ADM_DATE BETWEEN '$date1' AND '$date2' AND Student_Status='ACTIVE' ORDER BY ADM_DATE DESC");
