			...
			if ($checkAttendanceGen == true) {
				...
				if ($check_payslip_data) {
					...
					foreach ($employeeData as $key => $value) {
						...
						$present_days = $this->custom_lib->checkNoofPresentDays_new($total_days, $absent_days, $prev_month_total_days);
						...
						$result[$value['EMPID']] = array(
							...
							'hra_rent'		=> $HRA_RENT,
							'hra_elect'		=> $HRA_ELECT,
							'hra_security'		=> $HRA_SECURITY,
							'hra_garage'		=> $HRA_GARAGE,
							'ta_amount'		=> $ta_amount,
	...
	public function generateMonthlyPayslip()
	{
		...
		foreach ($employeeList as $key => $value) {
			...
			$data = array(
				...
				'hra_rent_deduct'		=> $payslipGenData[$value['EMPID']]['hra_rent'],
				'hra_security_deduct'	=> $payslipGenData[$value['EMPID']]['hra_security'],
				'hra_garage_deduct'		=> $payslipGenData[$value['EMPID']]['hra_garage'],
				'hra_elect_deduct'		=> $payslipGenData[$value['EMPID']]['hra_elect'],
