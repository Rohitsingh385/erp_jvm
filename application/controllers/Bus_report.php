...
class Bus_report extends MY_Controller
{
	...
	public function stu_buslist()
	{
		...
		if ($class == 'All' && $sec == 'All') {
			$data = $this->db->query("select stu.ADM_NO,stu.FIRST_NM,stu.FATHER_NM,stu.C_MOBILE,stu.DISP_CLASS,stu.DISP_SEC,stu.ROLL_NO,stu.STOPNO,(select STOPPAGE from stoppage where stu.STOPNO=STOPNO) stopname,stu.STOPNO,(SELECT AMT from stop_amt where stu.STOPNO=STOP_NO) stp_amt, stu.BUS_NO from student as stu where stu.STOPNO>1 AND stu.Student_status='ACTIVE' order by BUS_NO,CLASS,FIRST_NM")->result();
		} elseif ($sec == 'All') {
			$data = $this->db->query("select stu.ADM_NO,stu.FIRST_NM,stu.FATHER_NM,stu.C_MOBILE,stu.DISP_CLASS,stu.DISP_SEC,stu.ROLL_NO,stu.STOPNO,(select STOPPAGE from stoppage where stu.STOPNO=STOPNO) stopname,stu.STOPNO,(SELECT AMT from stop_amt where stu.STOPNO=STOP_NO) stp_amt,stu.BUS_NO from student as stu where stu.CLASS='$class' AND stu.STOPNO>1 AND stu.Student_status='ACTIVE' order by BUS_NO,CLASS,FIRST_NM")->result();
		} else {
			$data = $this->db->query("select stu.ADM_NO,stu.FIRST_NM,stu.FATHER_NM,stu.C_MOBILE,stu.DISP_CLASS,stu.DISP_SEC,stu.ROLL_NO,stu.STOPNO,(select STOPPAGE from stoppage where stu.STOPNO=STOPNO) stopname,stu.STOPNO,(SELECT AMT from stop_amt where stu.STOPNO=STOP_NO) stp_amt,stu.BUS_NO from student as stu where stu.CLASS='$class' AND stu.SEC='$sec' AND stu.STOPNO>1 AND stu.Student_status='ACTIVE' order by BUS_NO,CLASS,FIRST_NM")->result();
		}
	...
	public function stoppage_details()
	{
		...
		$data = $this->db->query("SELECT ADM_NO,FIRST_NM,FATHER_NM,C_MOBILE,DISP_CLASS,DISP_SEC,ROLL_NO from student where STOPNO='$stoppage' AND Student_status='ACTIVE' AND Class in ('$clsss') order by FIRST_NM")->result();
	...
	public function stoppage_summary_data()
	{
		...
		$data = $this->db->query("SELECT distinct STOPNO,BUS_NO,(select STOPPAGE from stoppage where stu.STOPNO=STOPNO) stopname,(SELECT AMT from stop_amt where stu.STOPNO=STOP_NO) stp_amt,(SELECT COUNT(*) FROM student WHERE student.STOPNO=stu.STOPNO AND student.STOPNO>1 AND Student_Status='ACTIVE')TOTALSTUDENT,(SELECT COUNT(*) FROM student WHERE student.STOPNO=stu.STOPNO AND student.sex=1 AND student.STOPNO>1 AND Student_Status='ACTIVE')MALE,(SELECT COUNT(*) FROM student WHERE student.STOPNO=stu.STOPNO AND student.sex=2 AND student.STOPNO>1 AND Student_Status='ACTIVE')FEMALE FROM `student` stu where stu.Student_status='ACTIVE' AND stu.STOPNO>1 ORDER by stu.BUS_NO,stu.STOPNO")->result();
		$array = array(
...
<?php
	...
	public function busno_details_old()
	{
	...
	public function busno_details()
	{
		...
		$GetMonthData = $this->dbcon->select('month_master', 'id', "month_name='$month_name'");
		foreach ($GetMonthData as $m_code) {
			$month_id = $m_code->id;
		}
		...
		if ($buscode == 'All') {
			...
			if ($trip == 'All') {
				// $data = $this->db->query("select s.adm_no,s.first_nm,s.father_nm,s.disp_class,s.disp_sec,s.c_mobile,s.stopno,st.stoppage,s.BUS_NO from student s inner join stoppage st on s.stopno=st.STOPNO where s.BUS_NO!='' order by st.stopno;")->result();
				// $data = $this->db->query("SELECT stf.NEW_STOPNO,stf.FROM_APPLICABLE_MONTH,stu.ADM_NO,stu.FIRST_NM,stu.FATHER_NM,stu.DISP_CLASS,stu.DISP_SEC,stu.C_MOBILE,st.stoppage, stf.BUS_NO FROM student_transport_facility stf JOIN student stu ON stu.ADM_NO=stf.ADM_NO join stoppage st on stf.NEW_STOPNO=st.STOPNO WHERE stf.NEW_STOPNO <> 1 AND stf.FROM_APPLICABLE_MONTH = '$month_name';")->result();
				$data = $this->db->query("SELECT stu.ADM_NO,stu.FIRST_NM,stu.FATHER_NM,stu.DISP_CLASS,stu.DISP_SEC,stu.C_MOBILE,st.stoppage, abd.BUS_NO FROM transport_adm_details abd JOIN student stu ON stu.ADM_NO=abd.admno join stoppage st on abd.stoppage_id=st.STOPNO WHERE  abd.month='$month_id' order by st.stopno")->result();
			} else {
				// $data = $this->db->query("select s.adm_no,s.first_nm,s.father_nm,s.disp_class,s.disp_sec,s.c_mobile,s.stopno,st.stoppage,s.BUS_NO from student s inner join stoppage st on s.stopno=st.STOPNO where s.bus_no!='' and s.class in ('$clsss') order by st.stopno")->result();
				$data = $this->db->query("SELECT stu.ADM_NO,stu.FIRST_NM,stu.FATHER_NM,stu.DISP_CLASS,stu.DISP_SEC,stu.C_MOBILE,st.stoppage, abd.BUS_NO FROM transport_adm_details abd JOIN student stu ON stu.ADM_NO=abd.admno join stoppage st on abd.stoppage_id=st.STOPNO WHERE  abd.month='$month_id' and stu.class in ('$clsss') order by st.stopno")->result();
			}
		} else {
			if ($trip == 'All') {
				// $data = $this->db->query("select s.BUS_NO,s.adm_no,s.first_nm,s.father_nm,s.disp_class,s.disp_sec,s.c_mobile,s.stopno,st.stoppage from student s inner join stoppage st on s.stopno=st.STOPNO where s.BUS_NO=$buscode")->result();
				// $data = $this->db->query("SELECT stf.NEW_STOPNO,stf.FROM_APPLICABLE_MONTH,stu.ADM_NO,stu.FIRST_NM,stu.FATHER_NM,stu.DISP_CLASS,stu.DISP_SEC,stu.C_MOBILE,st.stoppage, stf.BUS_NO FROM student_transport_facility stf JOIN student stu ON stu.ADM_NO=stf.ADM_NO join stoppage st on stf.NEW_STOPNO=st.STOPNO WHERE stf.NEW_STOPNO <> 1 AND stf.FROM_APPLICABLE_MONTH = '$month_name' and stf.BUS_NO=$buscode")->result();
				$data = $this->db->query("SELECT stu.ADM_NO,stu.FIRST_NM,stu.FATHER_NM,stu.DISP_CLASS,stu.DISP_SEC,stu.C_MOBILE,st.stoppage, abd.BUS_NO FROM transport_adm_details abd JOIN student stu ON stu.ADM_NO=abd.admno join stoppage st on abd.stoppage_id=st.STOPNO WHERE  abd.month='$month_id' and abd.BUS_NO=$buscode")->result();
			} else {
				//$data = $this->db->query("select s.BUS_NO,s.adm_no,s.first_nm,s.father_nm,s.disp_class,s.disp_sec,s.c_mobile,s.stopno,st.stoppage from student s inner join stoppage st on s.stopno=st.STOPNO where s.BUS_NO=$buscode and s.class in ('$clsss')")->result();
				// $data = $this->db->query("SELECT stf.NEW_STOPNO,stf.FROM_APPLICABLE_MONTH,stu.ADM_NO,stu.FIRST_NM,stu.FATHER_NM,stu.DISP_CLASS,stu.DISP_SEC,stu.C_MOBILE,st.stoppage, stf.BUS_NO FROM student_transport_facility stf JOIN student stu ON stu.ADM_NO=stf.ADM_NO join stoppage st on stf.NEW_STOPNO=st.STOPNO WHERE stf.NEW_STOPNO <> 1 AND stf.FROM_APPLICABLE_MONTH = '$month_name' and stf.BUS_NO=$buscode AND stu.class in ('$clsss')")->result();
				$data = $this->db->query("SELECT stu.ADM_NO,stu.FIRST_NM,stu.FATHER_NM,stu.DISP_CLASS,stu.DISP_SEC,stu.C_MOBILE,st.stoppage, abd.BUS_NO FROM transport_adm_details abd JOIN student stu ON stu.ADM_NO=abd.admno join stoppage st on abd.stoppage_id=st.STOPNO WHERE  abd.month='$month_id' and abd.BUS_NO=$buscode AND stu.class in ('$clsss')")->result();
			}
		}
	...
	public function download_busreport_bus_no_old()
	{
		...
		if ($buscode == 'All') {
			...
			if ($trip == 'All') {
				$data = $this->db->query("select s.adm_no,s.first_nm,s.father_nm,s.disp_class,s.disp_sec,s.c_mobile,s.stopno,st.stoppage,s.BUS_NO from student s inner join stoppage st on s.stopno=st.STOPNO where s.BUS_NO!='' order by s.bus_no,s.class,s.first_nm;")->result();
			} else {
				$data = $this->db->query("select s.adm_no,s.first_nm,s.father_nm,s.disp_class,s.disp_sec,s.c_mobile,s.stopno,st.stoppage,s.BUS_NO from student s inner join stoppage st on s.stopno=st.STOPNO where s.bus_no!='' and s.class in ('$clsss') order by s.bus_no,s.class,s.first_nm")->result();
			}
		} else {

			if ($trip == 'All') {
				$data = $this->db->query("select s.BUS_NO,s.adm_no,s.first_nm,s.father_nm,s.disp_class,s.disp_sec,s.c_mobile,s.stopno,st.stoppage from student s inner join stoppage st on s.stopno=st.STOPNO where s.BUS_NO=$buscode order by s.bus_no,s.class,s.first_nm")->result();
			} else {
				$data = $this->db->query("select s.BUS_NO,s.adm_no,s.first_nm,s.father_nm,s.disp_class,s.disp_sec,s.c_mobile,s.stopno,st.stoppage from student s inner join stoppage st on s.stopno=st.STOPNO where s.BUS_NO=$buscode and s.class in ('$clsss') order by s.bus_no,s.class,s.first_nm")->result();
			}
		}
	...
	public function busno_details_pdf()
	{
		$buscode = $this->input->post('buscode');
		$trip = $this->input->post('trip');
		// $buscode = $buscode-1;
	...
	public function busno_studetails()
	{
		...
		foreach ($stopno as $stop_details) {
			$data['stu_details'][$stop_details->stop_nm] = $this->db->query("SELECT DISP_CLASS,DISP_SEC, ADM_NO,FIRST_NM,ROLL_NO,FATHER_NM,C_MOBILE,(SELECT HOUSENAME FROM EWARD WHERE HOUSENO=STUDENT.EMP_WARD) WARD FROM STUDENT WHERE STOPNO = '$stop_details->stopno' ORDER BY BUS_NO,CLASS,SEC,FIRST_NM")->result();
		}
	...
	public function download_busreport()
	{
		...
		$stoppage_name = $stop_name[0]->STOPPAGE;
		$school_setting = $this->dbcon->select('school_setting', '*');
		$data = $this->db->query("select ADM_NO,FIRST_NM,FATHER_NM,C_MOBILE,DISP_CLASS,DISP_SEC,ROLL_NO,BUS_NO from student where STOPNO='$stoppage' AND Student_status='ACTIVE' order by BUS_NO, CLASS,FIRST_NM")->result();
	...
	public function download_busnoreport()
	{
		...
		$buscode = $this->input->post('stoppage_no');
