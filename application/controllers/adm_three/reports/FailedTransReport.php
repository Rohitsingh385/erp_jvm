...
class FailedTransReport extends MY_Controller {
	...
	public function index(){
		...
		$data['transFailedReportData'] = $this->alam->selectA('three_adm_data','*',"f_code <> 'Ok' and transaction_id is not null");
		$this->threeAdmissionAdminTemplate('adm_three/admin/transFailedReportData',$data);
	}
	...
	public function ReportStuData(){
		...
		$val = date('Y-m-d',strtotime($this->input->post('value')));
		$data['valdate'] = date('Y-m-d',strtotime($this->input->post('value')));
		$data['ReportData'] = $this->alam->selectA('three_adm_data','*',"date(created_at)='$val' AND f_code <> 'Ok' and transaction_id is not null");
	...
	public function ReportStuDateRange(){
		...
		$data['ReportData'] = $this->alam->selectA('three_adm_data','*',"date(created_at) BETWEEN '$start_date' AND '$end_date' AND f_code <> 'Ok' and transaction_id is not null");
	...
	public function ReportStuDateRangeStatus(){
		...
		$data['ReportData'] = $this->alam->selectA('three_adm_data','*',"date(created_at) BETWEEN '$start_date' AND '$end_date' AND verified_status='$verified_status' AND f_code='f'");
		$data['nur_reg_user'] = $this->alam->selectA('nur_reg_user','*',"id='$user_session_id'");
