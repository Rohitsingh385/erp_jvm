...
class NotAttempted extends MY_Controller {
	...
	public function index(){
		...
		$data['notAttemptedReportData'] = $this->alam->selectA('three_adm_data','*',"transaction_id is null");
		$this->threeAdmissionAdminTemplate('adm_three/admin/notAttemptedReportData',$data);
	}
	...
	public function ReportStuData(){
		...
		$val = date('Y-m-d',strtotime($this->input->post('value')));
		$data['valdate'] = date('Y-m-d',strtotime($this->input->post('value')));
		$data['ReportData'] = $this->alam->selectA('three_adm_data','*',"date(created_at)='$val' AND transaction_id is null");
	...
	public function ReportStuDateRange(){
		...
		$data['ReportData'] = $this->alam->selectA('three_adm_data','*',"date(created_at) BETWEEN '$start_date' AND '$end_date' AND transaction_id is null");
