...
class Payroll_details extends MY_Controller
{
	...
	public function update($id = null)
	{
		...
		$data['quarterData'] =$this->db->query("SELECT HRA_RENT,HRA_SECURITY,HRA_GARAGE FROM PAY_CONTROL WHERE employee_id=$id")->row_array();
		...
		$data['qtr_master'] = $this->sumit->fetchAllData('*', 'qtr_master', array());
	...
	public function updateProcess($id = null)
	{
		...
		$qtr_id=$this->input->post('quater_area');
		$resultData = $this->sumit->fetchAllData('*','qtr_master',"ID='$qtr_id'");
		$sqft=$resultData[0]['SQFT'];
		...
		$data = array(
			...
			'QUATER_AREA'		=> $sqft,
			 'QUARTER_RENT'		=> $this->input->post('house_rent'),
		...
		if ($this->input->post('maturity_date_3') != '') {
			$maturity_date3 = date('Y-m-d', strtotime($this->input->post('maturity_date_3')));
		}
	...
	function setRent()
	{
		...
		$resultData = $this->sumit->fetchAllData('*','qtr_master',"ID='$id'");
		$sqft=$resultData[0]['SQFT'];
		$rent=$resultData[0]['RENT'];
		$security=$resultData[0]['SECURITY'];
		$garage=$resultData[0]['GARAGE'];

		$array = array($sqft, $rent, $security,$garage);
