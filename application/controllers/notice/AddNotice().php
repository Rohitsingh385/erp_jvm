...
class Addnotice extends MY_Controller {
	...
	public function index(){
		...
		$data['noticeData'] = $this->alam->selectA('notice','*,(select distinct(DISP_CLASS) from student where CLASS=notice.class)disp_class,(select distinct(DISP_SEC) from student where SEC=notice.sec)disp_sec',"CLASS='$class' AND SEC='$sec' AND sent_status = 'S'");
	...
	public function saveNotice(){
		...
		$insertLastId = $this->db->insert_id();
			...
			foreach($stuData as $key => $val){
					...
					$msg = $notice;
					//$this->sms_lib->sendSMS($val['C_MOBILE'],$msg);
			}
