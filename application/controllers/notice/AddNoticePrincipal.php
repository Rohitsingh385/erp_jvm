...
class AddNoticePrincipal extends MY_Controller {
	...
	public function index(){
		...
		$data['noticeData'] = $this->alam->selectA('notice','*,(select distinct(DISP_CLASS) from student where CLASS=notice.class)disp_class,(select distinct(DISP_SEC) from student where SEC=notice.sec)disp_sec');
		...
		if($sendto == 1){
			...
			foreach($selectdata as $key => $val){
				...
				$mobile = $teacherData[0]['C_MOBILE'];
					...
					$this->alam->insert('notice_adm_wise',$saveAllStu);
					$msg = $notice;
					//$this->sms_lib->sendSMS($val2['C_MOBILE'],$msg);
					}
				}	
			}	
