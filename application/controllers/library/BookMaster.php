...
class BookMaster extends MY_controller{
	...
	{
		$data['BookMasterData'] = $this->alam->selectA('bookmaster', '*', "1='1' limit 100");
		$cc = $this->alam->selectA('bookmaster', "count('id')cnt", "1='1' limit 10");
		$data['cnt'] = $cc[0]['cnt'];
		// echo "<pre>";print_r($data);die;
		$this->render_template('library/book_master');
	}

	public function api_BookMaster()
	{
		$postData = $this->input->post();
		// echo "<pre>";print_r($postData);die;
		$data = $this->ajax->fetchAllData($postData);
		// echo "<pre>";print_r($data);die;
	...
	public function countSubject(){
		...
		$accData    = $this->alam->selectA('bookmaster','max(id)max');
