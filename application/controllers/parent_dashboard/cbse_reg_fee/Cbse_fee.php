...
class Cbse_fee extends MY_controller
{
	...
	public function cbse_registration()
	{
		...
		$session = array(
			'adm'             => $adm_no,
			'id'              => $stu_data[0]->id,
			'name'            => $stu_data[0]->SName,
			'class'           => $stu_data[0]->class,
			'sec'             => $stu_data[0]->sec,
			'disp_sec'        => $stu_data[0]->disp_sec,
			'roll'            => $stu_data[0]->roll,
			'set_amt'         => $stu_data[0]->Registration_Charges,
			'fee_amt_service' => $stu_data[0]->Processing_ICard_Charges,
			'total' => $stu_data[0]->Total,
			'mobile'          => $stu_data[0]->mobile,
			'f_code'          => $stu_data[0]->F_Code
		);
		...
		//    $this->load->view('parents_dashboard/cbse_reg_fee/cbse_registration',$data);
	...
	public function payNow()
	{
		...
		if (!empty($generate_session)) {
			$adm_no = $this->session->userdata('adm');
			$allData = $this->alam->selectA('cbse_reg_amount', '*', "AdmNo='$adm_no'");
			//echo $this->db->last_query();die;
			$session = array(
				'adm'             => $allData[0]['AdmNo'],
				'id'              => $allData[0]['id'],
				'name'            => $allData[0]['SName'],
				'class'           => $allData[0]['class'],
				'sec'             => $allData[0]['Class_Sec'],
				'disp_sec'        => $allData[0]['disp_sec'],
				'roll'            => $allData[0]['roll'],
				'fee_amt'         => $allData[0]['Registration_Charges'],
				'fee_amt_service' => $allData[0]['Processing_ICard_Charges'],
				'fee_amt_total'   => $allData[0]['Total'],
				'mobile'          => $allData[0]['mobile'],
				'f_code'          => $allData[0]['f_code']
			);
			// echo '<pre>';
			// print_r($session);
			// echo '</pre>';
		...
		$data['allData'] = $this->alam->selectA('cbse_reg_amount', '*', "id='" . $generate_session['id'] . "'");
		//die;
		$this->load->view('parents_dashboard/Cbse_Reg/payment/payNow_x', $data);
	}
