...
class Fees_master extends MY_controller{
	...
	public function edit_busmaster($id){
		...
		$stoppage_category_master = $this->dbcon->select('stoppage_category_master','*');
		$array = array(
			'data' => $data,
			'bus_no' => $bus_no,
			'stoppage_category_master' => $stoppage_category_master
		);
		// echo "<pre>";
		// print_r($array);
		// exit;
	...
	public function busmaster_update()
	{
		...
		$stop_name = strtoupper($this->input->post('stop_name'));

		$data_match = $this->dbcon->select('stoppage', 'STOPPAGE', "STOPPAGE='$stop_name'");
		...
		if ($fin_name == $stop_name) {
			$flag = 1;
		} 
		if ($flag == 1) {
			$this->session->set_flashdata('msg', "This ".$fin_name." is already exists , Amount is Updated Successfully");
			redirect('Fees_master/Bus_Stoppage_Master');
		} else {
			$this->session->set_flashdata('msg', "Successfully update");
			redirect('Fees_master/Bus_Stoppage_Master');
	...
	public function selectStoppage()
	{
		...
		$data = $this->dbcon->select('stoppage', 'STOPPAGE,STOPNO', "BUS_NO='$busno'");
...
<?php
	...
	public function stoppage_amt()
	{
		$group = $this->input->post('val');
		$amt = $this->dbcon->select('stoppage_category_master', 'Amt', "Stoppage_Group='$group'");
