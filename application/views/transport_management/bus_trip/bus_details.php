				...
				<tbody>
					...
					if ($bus_trip) {
						...
						foreach ($bus_trip as $bus_masterd) {
							$arr = '';
							$trip_class = unserialize($bus_masterd->Class_No);
							$destination_array = implode(',', $trip_class); //1,2,4

							$cls_nm = $this->db->query("SELECT CLASS_NM FROM classes WHERE Class_No IN ($destination_array)")->result();
							foreach ($cls_nm as $cl_nm) {
								$arr .= $cl_nm->CLASS_NM . ', ';
							}
