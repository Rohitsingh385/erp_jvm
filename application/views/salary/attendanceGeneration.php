                  ...
                  <?php if (isset($result)) { ?>
                    <div class="row text-center monthly_attendance">
                      <div class="col-sm-12">
                        ...
                        <br>
                        Attendance Cycle From <strong><?php echo date('d-M-Y', strtotime($chunkData['start_cycle'] . '-' . $chunkData['current_month'] . '-' . $chunkData['current_year'])) . ' </strong>To<strong> ' . date('d-M-Y', strtotime($chunkData['end_cycle'] . '-' . $chunkData['current_month'] . '-' . $chunkData['current_year'])); ?></strong>
                      </div>
                    </div>
                    <hr>
                        ...
                        <table class="table table-bordered dataTable table-striped table-hover">
                          ...
                          <tbody>
                            ...
                            foreach ($result as $key => $value) {
                              ...
                              $total_present = $this->custom_lib->checkNoofPresentDays_new($chunkData['current_month_total_days'], $total_absent);
                            ?>
                              <tr>
                                ...
                                ?>
                                  <td class="<?php echo $value['EMPID'] . '_' . $valll; ?>"><?php if ($value[$val] === 'P') {
                                    ...
                                    <?php  } elseif ($value[$val] === 'H') { ?>
                                      ...
                                      <span class="label label-warning" data-toggle='tooltip' title="Holiday" onclick="funApprove('<?php echo $value['EMPID']; ?>','<?php echo $valll; ?>','Holiday','<?php echo $value['id']; ?>','<?php echo $key; ?>')"><?php echo $value[$val]; ?></span>

                                    <?php } elseif ($value[$val] === 'LWP') { ?>

                                      <span class='label label-danger' data-toggle='tooltip' title="Leave Without Pay" onclick="funApprove('<?php echo $value['EMPID']; ?>','<?php echo $valll; ?>','Leave Without Pay','<?php echo $value['id']; ?>','<?php echo $key; ?>')"><?php echo $value[$val]; ?></span>

                                    <?php } elseif ($value[$val] === 'EL' || $value[$val] === 'CL' || $value[$val] === 'ML' || $value[$val] === 'DDL' || $value[$val] === 'HPL') { ?>

                                      <span class='label label-info' data-toggle='tooltip' title="Leave" onclick="funApprove('<?php echo $value['EMPID']; ?>','<?php echo $valll; ?>','Leave','<?php echo $value['id']; ?>','<?php echo $key; ?>')"><?php echo $value[$val]; ?></span>
  ...
  function funApprove(emp_id, selected_date, status, empid_no, key) {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (response.msg == 2) {
          swal({
            title: "Please Generate Attendance First",
            text: "Please Generate Attendance First",
            type: "info"
          });
        } else {
