...
<ol class="breadcrumb">
	<li class="breadcrumb-item"><a href="#">Mock Exam</a> <i class="fa fa-angle-right"></i></li>
</ol>
...
<script>
	function classes(val) {
		$.post("<?php echo base_url('Marks_entry/classess3'); ?>", {
			val: val
		}, function(data) {
	...
	function secc(val) {
		...
		$.post("<?php echo base_url('Marks_entry/section3'); ?>", {
			val: val,
			Class_No: Class_No
		}, function(data) {
			$("#exm_typ").html(data);
	...
	function exam_type(ExamCode) {
		...
		$.post("<?php echo base_url('marks_entry/subject3'); ?>", {
			ExamCode: ExamCode,
			Class_No: Class_No,
			sec: sec,
			ExamMode: ExamMode
	...
	function sorybyview(val) {
		...
		$.post("<?php echo base_url('Marks_entry/stu_list3'); ?>", {
			sortval: sortval,
			opt_code: opt_code,
			Class_No: Class_No,
			sec: sec,
	...
	function marks(value) {
		...
		if ((MaxMarks >= vall) || (vall == 'ab') || (vall == 'AB')) {
			...
			var mxm = splt[2];
			$.post("<?php echo base_url('Marks_entry/sv_nd_upd'); ?>", {
				adm_no: adm_no,
				exm_typ: exm_typ,
				subcode: subcode,
				classs: classs,
