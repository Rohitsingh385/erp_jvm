...
<script>
	 ...
	 var ExamMode = $("#ExamMode").val();
	  ...
	  $.post("<?php echo base_url('marks_entry/subject2'); ?>",{ExamCode:ExamCode,Class_No:Class_No,sec:sec,ExamMode:ExamMode},function(data){
		 ...
		 $("#view_max_markss").val(fill[1]);
		  ...
		  $.post("<?php echo base_url('marks_entry/subject'); ?>",{ExamCode:ExamCode,Class_No:Class_No,sec:sec,ExamMode:ExamMode},function(data){
		 var fill = $.parseJSON(data);
		 $("#sub").html(fill[0]);
		 $("#view_max_markss").val(fill[1]);
	 });
  ...
  function marks(value){
	...
	function marksformsc(value) {
		var val = value.id; //marks_1
		var splt = val.split("_");
		var spltval = splt[1]; //1
		var vall = $("#marks_" + spltval).val(); //a
		var mxmrks = $("#view_max_marks").text();
		var splt = mxmrks.split(" ");
		var MaxMarks = Number(splt[2]); //50
		// alert(MaxMarks);
		...
		var entr_val = $("#marks_" + spltval).val();
		var mxm = splt[2];
		$.post("<?php echo base_url('Marks_entry/sv_nd_upd_for_msc'); ?>", {
			adm_no: adm_no,
			exm_typ: exm_typ,
			subcode: subcode,
			classs: classs,
