...
<ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="#">Personality Assessment Grade</a> <i class="fa fa-angle-right"></i></li>
</ol>
...
<script>
  ...
  function classes(val){
	  $.post("<?php echo base_url('perGrade/classess'); ?>",{val:val},function(data){
  ...
  function secc(val){
    ...
    var sec    = $("#sec").val();
    var trm    = $("#trm").val();
	$.post("<?php echo base_url('perGrade/stu_list'); ?>",{classs:classs,disp_classs:disp_classs,sec:sec,trm:trm},function(data){
		$("#loading2").hide();
		$("#stu_list").html(data);
	});
  }
  ...
  function co_sch(skill_id,adm_no,grade){
	...
	var sec    = $("#sec").val(); 
	var trm    = $("#trm").val();
	$.post("<?php echo base_url('perGrade/save_upd'); ?>",{adm_no:adm_no,classs:classs,sec:sec,skill_id:skill_id,grade:grade,trm:trm},function(data){
		//alert(data);
	});
  }
</script>
