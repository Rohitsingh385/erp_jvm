...
<form id="" target="_blank" method="post" action="
	<?php 
		  if($classs=='11' || $classs == '12'){
			  //echo base_url('report_card/report_card/generatePDF_ix_x');
			  echo base_url('report_card/report_card/generatePDF_ix_new');
		  }
		  ...
		  elseif($classs=='6' || $classs == '7' || $classs == '5'){
			  echo base_url('report_card/ThreeToFive_annual');
		  }
		  elseif($classs=='8' || $classs == '9' || $classs == '10'){
			  echo base_url('report_card/report_card/generatePDF_VI_VIII_T1_new');
		  }
		  else{
				...
				<td>
					<?php
						if($data->t2_report_card_status == 1){
							...
							if($trm == 1){
								?>
									<a download href='<?php echo base_url('report_card/annual/'.$adm.'.pdf'); ?>' class='btn btn-success btn-sm'> Download </a>
								<?php
							}else{
								?>
									<a download href='<?php echo base_url('report_card_annual/'.$adm.'.pdf'); ?>' class='btn btn-success btn-sm'> Download </a>
								<?php
							}
						}
					?>
