...
<table style="width:100%">
	<tr>
		...
		<td>
			<center>
				<h1><span style="color:#02933e;font-size:24px !important;">JAWAHAR VIDYA MANDIR</span></h1>Shyamali Colony, Doranda, Ranchi-834002<br />Session- ( 2023-2024 )<br />CROSSLIST REPORT OF <?php echo $classnm[0]->CLASS_NM . '-' . $secnm[0]->SECTION_NAME; ?>&nbsp;&nbsp;<?php if($term==1){ echo 'MID TERM'; }else{ echo 'END TERM'; } ?>
			</center>
		</td>
		<td>
			...
			<tr>
				...
				if($term == 1){
					...
					?>
						...
						<td align="center"><?php echo round(((($sub->subject_code != 15)?$marks_pt->M3:$marks_pt->M3*2) + ($marks_rt->M3/2) + $marks_hf->M3),0); ?></td>
					<?php	}
				}else{
					foreach ($subject_list as $key => $sub) {
						$marks_pt = $this->rc->get_marks_grade_for_consolidated_analysis_new_iii($p->CLASS, $p->SEC, $sub->subject_code, $p->ADM_NO, 1,$trm);
