...
<table border='1' class='table'>
  ...
  <?php
    $stuData = $this->alam->selectA('student','ADM_NO,ROLL_NO, `CLASS`,(SELECT ExamMode FROM classes WHERE Class_No=student.CLASS)examode,DISP_CLASS,DISP_SEC,SEC,FIRST_NM,MIDDLE_NM,Height,Weight',"CLASS='$class' AND SEC='$sec' AND Student_Status='ACTIVE' order by ROLL_NO");
	$adm_arr = array();
	$class_arr = array();
	$sec_arr = array();
	$first_nm_arr = array();
	$roll_no_arr = array();
	$tot_wet_mrk_arr = array();
	$tot_per_arr = array();
	$tot_grd_arr = array();
	$tot_mo_arr = array();
	$grd_arr = array();
	foreach($stuData as $key => $val){
		...
		$admno       = $val['ADM_NO'];
		$disp_class  = $val['CLASS'];
		$disp_sec    = $val['SEC'];
		
		$adm_arr[]   = $val['ADM_NO'];
		$class_arr[] = $val['CLASS'];
		$sec_arr[]       = $val['SEC'];
		$first_nm_arr[] = $val['FIRST_NM'];
		$roll_no_arr[]  = $val['ROLL_NO'];
		//for attendance //
  ...
  <?php
	$attendance_arr[] = $tot_present_day."/".$tot_working_day;
  ?>
  <tr>
    <td><?php echo $val['ADM_NO']; ?><input type='hidden' name='admm_no[]' value='<?php echo $val['ADM_NO']; ?>'></td>
	...
	<input type='hidden' name='class_id' value='<?php echo $disp_class; ?>'>
	<input type='hidden' name='sec[]' value='<?php echo $disp_sec; ?>'>
	<input type='hidden' name='sec_id' value='<?php echo $disp_sec; ?>'>
		  ...
		  <tr>
			...
			<?php
			  ...
			  foreach($subjectList as $key3 => $val3){
				...
				$marks = $this->alam->selectA('marks','*',"Term='TERM-1' AND classes = '$disp_class' AND sec='$disp_sec' AND admno='$admno' AND SCode = '$subject_code' AND ExamC ='$ExamCode'");
				...
				if($pt_type == 1){
					if($ExamCode == 1){ //for PT
					  ...
					  foreach($pt_marks as $key4 => $val4){
                        $M3[]         = $val4['M3'];
                        $wetage_obt[] = $val4['M3']/$val4['M1']*$wetage1;
					  }					  
					  rsort($M3);
					  rsort($wetage_obt);
					  ...
					  if($M3 > 0 && $M1 > 0){
					   //$tot_wetage = ($round == 1) ? round (number_format(($M3/$M1)*$wetage1,2)) : number_format(($M3/$M1)*$wetage1,2);
						if($opt_code == 1){
							  $tot_wetage = ($M2 == 'AB' || $M2 == '-')?$M2:($M3/$M1)*$wetage1;
						  }else{
							 $tot_wetage = ($round == 1) ? round (number_format(($M3/$M1)*$wetage1,2)) : number_format(($M3/$M1)*$wetage1,2);	 
						  }
					  }else{
			...
			<td>
			  <?php 
			    ...
			    echo ($round == 1)?round($totPercent):number_format($totPercent,2); 
			  ?>
			  <!-- grade -->
			  <?php
		  ...
		  ?>
		    <td>
			 ...
			 if($val5['opt_code'] != 1){
			  ...
			  echo !empty(($totalSubjWetage[$val5['subject_code']]*100)/80) ? ($totalSubjWetage[$val5['subject_code']]*100)/80 : 0;	 
			 }			  
			 ?>
			</td>
		  <?php
		    ...
		    <td>
			  ...
			  //$totm = isset($totalSubjWetage[$val6['subject_code']]) ? $totalSubjWetage[$val6['subject_code']] : 0; 
			     if($val6['opt_code'] != 1){
				  ...
				  $totm = !empty(($totalSubjWetage[$val6['subject_code']]*100)/80) ? ($totalSubjWetage[$val6['subject_code']]*100)/80 : 0;	 
				 }			  
		  ...
		  <tr>
			...
			<?php
			  ...
			  foreach($subjectList as $key3 => $val3){
				...
				$marks = $this->alam->selectA('marks','*',"Term='TERM-2' AND classes = '$disp_class' AND sec='$disp_sec' AND admno='$admno' AND SCode = '$subject_code' AND ExamC ='$ExamCode'");
			...
			<td>
			  <?php 
			    ...
			    echo ($round == 1)?round($totPercentt):number_format($totPercentt,2); 
			  ?>
			  <!-- grade -->
			  <?php
			    $grade = $this->alam->selectA('grademaster','CRange,ORange,Grade,Qualitative_Norms');
		    ...
		    <td>
			  <?php 
				...
				if($val5['opt_code'] != 1){
				  ...
				  echo !empty(($totalSubjWetagee[$val5['subject_code']]*100)/80) ? ($totalSubjWetagee[$val5['subject_code']]*100)/80 : 0;	 
				 }	
			  ?>
			</td>
		  <?php
		  ...
		  ?>
		    <td>
			  ...
			  if($val6['opt_code'] != 1){
				...
				$totm = !empty(($totalSubjWetagee[$val6['subject_code']]*100)/80) ? ($totalSubjWetagee[$val6['subject_code']]*100)/80 : 0;	 
			  }
 ...
 <?php } ?>
	 <?php
		...
		$array_session = array(
			...
			'class_arr'       => $class_arr,
			'sec_arr'         => $sec_arr,
			'first_nm_arr'    => $first_nm_arr,
			'roll_no_arr'     => $roll_no_arr,
			'tot_wet_mrk_arr' => $tot_wet_mrk_arr,
			'tot_per_arr'     => $tot_per_arr,
			'tot_grd_arr'     => $tot_grd_arr,
			'attendance_arr'  => $attendance_arr,
			'tot_mo_arr'      => $tot_mo_arr,
			'grd_arr'         => $grd_arr,
			'subj_nm_arr'     => $subj_nm_arr,
		);
		
		$this->session->set_userdata('array_session',$array_session);
	 ?>
</table>
</div>
</form>