...
<div class='row'>
	...
	<div class='col-sm-10 col-md-10'>
				...
				foreach($classWiseQuestions as $key => $val){
					if($val['que_no'] == 1){
						...
						?>
							...
							<span style='font-size:18px;'><?php echo $val['question']; ?> <b>(MM-<?php echo $val['max_marks']; ?>)</b><input type='hidden' name='qusId' value='<?php echo $val['id']; ?>'></span>
							<br />
							<?php if($que_img != ''){ ?>
						...
						?>
							...
							<?php } ?>
							<center><button type='submit' id='btn' class='btn btn-success' onclick="saveAns()"><i class="fa fa-circle-o-notch fa-spin" id='process' style='display:none;'></i> Save Answer</button><button class='btn btn-info pull-right' type='button' onclick="saveStatus(<?php echo $val['e_exam_questions_hw_id']; ?>)" style='background:#3c8dbc;'>SUBMIT HOMEWORK</button></center>
						<?php
					}
				}
			?>
			...
			<b style='color:red'>4. .jpg, .jpeg,. .png File format allowed for uploading with max. size of 1000KB.</b><br/>
	</div>
	<div class='col-sm-1 col-md-1'></div>
</div><br /><br />
</div>
