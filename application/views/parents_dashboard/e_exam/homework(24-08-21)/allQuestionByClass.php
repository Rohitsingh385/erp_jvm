				...
				foreach($classWiseQuestions as $key => $val){
					if($val['que_no'] == 1){
						...
						?>
							...
							<br />
							<?php if($que_img != ''){ ?>
							<a href="<?php echo base_url($que_img); ?>" download><img src='<?php echo base_url($que_img); ?>' class='img-responsive' style='width:100px; height:100px;'></a><br />
							<?php } ?>
					    <?php
						?>
...
<script>
	...
	$(".filePHOTO").change(function(){
		...
		var ext = $('.filePHOTO').val().split('.').pop().toLowerCase();
			if(file_size > 5048576 || (ext != 'jpg') && (ext != 'jpeg') && (ext != 'png')){
