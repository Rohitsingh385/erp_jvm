                  ...
                  <table class='table table-bordered table-striped dataTable'>
                    <thead>
                      <tr>
						...
						<th class="thead-color text-center">Height</th>
                        <th class="thead-color text-center">Weight</th>
							...
							foreach($data as $key=>$value){
								?>
									<tr>
										...
										<td><input type="text" class="form-control"  onkeypress="return event.charCode>=48 && event.charCode<=57 || event.charCode==46" id="<?php echo $value->ADM_NO; ?>" onchange="heightwt(this.value,this.id)" value="<?php echo $value->Height; ?>" ></td>

										<td><input type="text" class="form-control"  onkeypress="return event.charCode>=48 && event.charCode<=57 || event.charCode==46" id="<?php echo $value->ADM_NO; ?>" onchange="weightwt(this.value,this.id)" value="<?php echo $value->Weight; ?>" ></td>
...
<script>
  ...
  function changegenger(gen_data){
	...
	function heightwt(val,value){
		if(val ==""){
			
		}else{
			$.ajax({
				url : "<?php echo base_url('bulk_updation/Classsecwise/height'); ?>",
				method : "POST",
				data : {val:val,value:value},
				success:function(data)
				{
					if(data == 1){
						...
						alert("Height is not updated");
					}
				},
			});
		}
	}
	function weightwt(val,value){
	
		if(val ==""){
			
		}else{
			$.ajax({
				url : "<?php echo base_url('bulk_updation/Classsecwise/weight'); ?>",
				method : "POST",
				data : {val:val,value:value},
				success:function(data)
				{
