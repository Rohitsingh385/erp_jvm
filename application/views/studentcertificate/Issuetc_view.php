...
<?php if (!empty($stu_list)) { ?>
    ...
    <div id="inrange">
        <div class="row col-md-offset-2">
            <div class="col-md-3">
                <div class="form-group">
                    <label for="admno">From Admission No.</label>
                    <input type="text" name="adm_no_one" id="adm_no_one" value="" class="form-control">
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group">
                    <label for="admno">To Admission No.</label>
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <br>
                    <input type="submit" class="btn btn-success" value="Generate PDF" formtarget="_blank">
                </div>
            </div>
        </div>
        <?php echo form_close(); ?>
                    ...
                    <table class="table table-bordered" id="ex_mod">
                        <thead>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="lschoold" id="lschoold" class="datePickerlsd" placeholder="set date" Size="7">
                                    <button onclick="saveDatelschooldate()" name="submit" id="submit" class="btn-success btn-sm">SET DATE</button>
                                </td>
                                ...
                                <td style="width:100%">
                                    <input type="text" name="ecdd" id="ecdd-" class="datePickerpassyear" placeholder="set date" Size="7"><br>
                                    <button onclick="SaveDatePassYear()" name="submit" id="submit" class="btn-success btn-sm">SET DATE</button>
                                </td>

                                <td></td>

                                <td>
                                    <input type="text" name="cid" id="cid-" class="datePickercid" placeholder="set date" Size="7">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select name='admit_class' id='admit_class-<?php echo $i; ?>'>
                                            <option value=''><?php echo $b->class_admitted; ?></option>
                                            <?php
                                    ...
                                    <td><input type="text" name="lschool" id="lschool-<?php echo $i; ?>" class="datePickerls" value="<?php echo empty($b->left_school) ? "" :  date("d-m-Y", strtotime($b->left_school)); ?>" Size="20px" style="height:50px;"></td>

                                    <td><input type="text" name="sclass" id="sclass-<?php echo $i; ?>" value="<?php echo $b->studied_class; ?>" Size="20px" style="height:50px;"></td>

                                    <td><!-- <input type="text" name="acad_year" id="acad_year" value="<?php echo $b->session_year; ?>" Size="20px"> -->

                                        <select name='acad_year' id='acad_year-<?php echo $i; ?>'>
                                            <option value=''><?php echo $b->session_year; ?></option>
                                            <?php
                                            foreach ($sess_list as $data) {
                                            ?>
 ...
 <!-- AISSE -->
                                    <td>
                                        ...
                                        <?php if ('PASSED AISSCE CBSE' == $b->status) { ?>
                                            <select name='syear' id='syear-<?php echo $i; ?>'>

                                                <option value='none'><?php if ("passed_aisse_cbse" == $b->status) {
                                                                            echo 'selected="selected"';
                                                ...
                                                <option value="passed_aisse_cbse" <?php if ("passed_aisse_cbse" == $b->status) {
                                                                                        echo ' selected="selected"';
                                                                                    } ?>> PASSED AISSCE 2nd ATTEMPT </option>
                                                <option value="passed_aisse_cbse" <?php if ("passed_aisse_cbse" == $b->status) {
                                                                                        echo ' selected="selected"';
                                                                                    } ?>> PASSED AISSCE AS PRIVATE CANDIDATE</option>
                                                <option value="passed_aisse_cbse" <?php if ("passed_aisse_cbse" == $b->status) {
                                                                                        echo ' selected="selected"';
                                                                                    } ?>> AISSCE APPEARED BUT NOT ATTEMPT </option>
                                                <option value="passed_aisse_cbse" <?php if ("passed_aisse_cbse" == $b->status) {
                                                                                        echo ' selected="selected"';
                                                                                    } ?>> AISSCE REGISTERED BUT NOT APPEARED </option>

                                            </select>
                                        <?php } ?>

                                        <?php if ('PASSED AISSE CBSE' == $b->status) { ?>
                                            <select name='syear' id='syear-<?php echo $i; ?>'>
                                                ...
                                                <option value="passed_aisse_cbse" <?php if ("passed_aisse_cbse" == $b->status) {
                                                                                        echo ' selected="selected"';
                                                                                    } ?>> PASSED AISSE 2nd ATTEMPT </option>
                                                <option value="passed_aisse_cbse" <?php if ("passed_aisse_cbse" == $b->status) {
                                                                                        echo ' selected="selected"';
                                                                                    } ?>> PASSED AISSE AS PRIVATE CANDIDATE</option>
                                                <option value="passed_aisse_cbse" <?php if ("passed_aisse_cbse" == $b->status) {
                                                                                        echo ' selected="selected"';
                                                                                    } ?>> AISSE APPEARED BUT NOT ATTEMPT </option>
                                                <option value="passed_aisse_cbse" <?php if ("passed_aisse_cbse" == $b->status) {
                                                                                        echo ' selected="selected"';
                                                                                    } ?>> AISSE REGISTERED BUT NOT APPEARED </option>
                                            </select>
                                        <?php } ?>

                                    </td>

                                    <td><input type="text" name='tgl' id="datepicker-<?php echo $i; ?>" class="datePickercd" value="<?php echo empty($b->cer_issue_date) ? "" :  date("d-m-Y", strtotime($b->cer_issue_date)); ?> " Size="20px" style="height:50px;">
                                    </td>
...
<script>
    ...
    function saveDatelschooldate() {
        var ldate = $('.datePickerlsd').val();
        var cls = $('#hclassr').val();
        var year = $('#syear').val();
        //alert(ldate);
        //alert(cls);
        //alert(year);
        $.ajax({
            url: '<?php echo site_url('studentcertificate/issuetc/update_lastschooldate') ?>',
            method: "POST",
            data: {
                "ldate": ldate,
                "cls": cls,
    ...
    function SaveDatePassYear() {
        var passyear = $('.datePickerpassyear').val();
        var cls = $('#hclassr').val();
        var year = $('#syear').val();
        $.ajax({
            url: '<?php echo site_url('studentcertificate/issuetc/update_passedyear') ?>',
            method: "POST",
            data: {
                "passyear": passyear,
                "cls": cls,
    ...
    function SaveIssueDate() {
        ...
        var year = $('#syear').val();
        $.ajax({
            url: '<?php echo site_url('studentcertificate/issuetc/update_issuedate') ?>',
            method: "POST",
            data: {
                "issuedate": issuedate,
                "cls": cls,
