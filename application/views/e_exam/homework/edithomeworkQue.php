		...
		<table class='table'>
			...
			<tr id='row_<?php echo $key; ?>'>
				<td id='load_topic' colspan='5'>
					...
					<div class='row'>
						<div class='col-sm-4'>
							<input type='text' placeholder='Max Marks' class='form-control' name='max_marks[]' value='<?php echo $val['max_marks']; ?>' required>
						</div>
						<div class='col-sm-4'>
							<?php
								if($val['que_img'] != ''){
						...
						<?php
							if($key != 0){
						?>
							<i class="fa fa-trash pull-right" style='color:red; cursor:pointer' title='DELETE' onclick="queDel(<?php echo $val['id']; ?>,<?php echo $val['e_exam_questions_hw_id']; ?>,<?php echo $key; ?>)"></i>
						<?php } ?>
					</div>
					<br />
				</td>
			...
			<tr>
				...
				<li> Press <button type='button' class='btn btn-success btn-xs'><i class="fa fa-plus" style='color:#fff;'></i></button> To Add New Question.</li>
...
<script type="text/javascript">
 ...
 function imgValidaye(id){
	 ...
	 var ext = $('#filePHOTO_'+id).val().split('.').pop().toLowerCase();
		if(file_size > 5048576 || (ext != 'jpg') && (ext != 'jpeg') && (ext != 'png') && (ext != 'pdf')){
			$.toast({
				heading: 'Error',
				text: 'File size must be less than 5000kb and only allowed jpg,jpeg,png format',
				showHideTransition: 'slide',
				icon: 'error',
				position: 'top-right',
			});
 ...
 function queDel(id,e_exam_questions_hw_id,key){
	 var cnf = confirm("Are you sure want to delete?");
	 if(cnf == true){
		$.ajax({
			url: "<?php echo base_url('e_exam/homework/AddHomework/delQuestion'); ?>",
			type: "POST",
			data: {id:id,e_exam_questions_hw_id:e_exam_questions_hw_id},
			success: function(ret){
				if(ret == 1){
					$("#row_"+key).hide();
					$.toast({
