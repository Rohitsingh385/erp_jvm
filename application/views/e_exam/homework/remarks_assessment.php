 ...
 ?>
   ...
   <div class="table-responsive">
		 	<table class='table'>
				...
				$mm = $this->pawan->selectA('e_exam_questions_hw_append','max_marks',"id='$qi'");
				$mxm = $mm[0]['max_marks'];
				?>
				...
				<td style="border: 1px solid ;width: 230px;"><?=$vals['question'];?>&nbsp;&nbsp;<span style="float:right;"><?php if($q_image){?><img src="<?=base_url().$q_image;?>" style="width:50px;height:50px;float:right;"><?php } ?></span> <br /><b>(MM-<?php echo $mxm; ?>)</b></td>
				...
				<input type="text" name="remarks" <?php if($TST==2){ echo 'readonly';}else{}?> style="background-color:#e6f2fd" value="<?php echo $vals['remarks']; ?>" placeholder="----" id="remarks_<?=$vals['id']?>" onchange="remarksd(this.value,'<?=$vals['id'];?>')" class="form-control"><br />
				
				<input type="text" name="mm" <?php if($TST==2){ echo 'readonly'; }else{} ?> value="<?php echo $vals['marks_by_teacher']; ?>" placeholder='Enter Marks' id="mm_<?=$vals['id']?>" onchange="maxmarks(this.value,'<?=$vals['id'];?>','<?=$mxm; ?>')" class="form-control">	
				</td>
				</tr>
				<?php } ?>
...
$(document).on('keypress', 'input,select', function (e) {
	...
	function maxmarks(value,qid,mm){
		var mm = Number(mm);
		if(mm >= value){
			var qid		=	qid;		
			var mm	=	 $('#mm_'+qid).val();		
			$.post("<?php echo base_url('e_exam/homework/RemarksAssessment/marks_entry'); ?>",{qid:qid,remarks:mm},function(data){
				alert_msg('','Remarks Recorded...!','success');
			});
		}else{
			var mm	=	 $('#mm_'+qid).val('');	
			alert_msg('','Invalid Entry...!','error');
		}
	}
