     ...
     <form id="searchForm" method="post" action="<?php echo base_url('salary_report/monthlypf_report'); ?>">
      ...
      <?php if(isset($resultList) && !empty($resultList)){ ?>
        <div class="text-center">
          <a href="<?php echo base_url('salary_report/Monthlypf_report/generatePDFReport/'.$year.'/'.$month); ?>" class="btn btn-success" target="_blank"><i class="fa fa-file-pdf-o"></i> Generate PDF Report</a>
        </div>
              ...
              <tbody>
                  ...
                  foreach ($resultList as $key => $value) {  $eps_wages=0; ?>
                      <tr>
                        ...
                        <td class="text-right"><?php if ($value['pension_applied']==1){ echo $eps_cont = round(($eps_wages*$value['pension_rate'])/100);}else{ echo $eps_cont=0;} ?></td>
