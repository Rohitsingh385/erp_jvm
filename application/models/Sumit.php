...
class Sumit extends CI_model{
	...
	public function getEmpDAta_new1($time_from,$WING_MASTER_ID,$desig)
	{
		$query = $this->db->query("SELECT e.id, e.EMPID, e.EMP_FNAME, e.EMP_MNAME, e.EMP_LNAME , e.DESIG , e.WING_MASTER_ID,
		(SELECT ATT_STATUS FROM daily_emp_att d WHERE d.EMPID=e.EMPID AND d.DATE='$time_from')AS ATT_STATUS,
		(SELECT IN_TIME FROM daily_emp_att d WHERE d.EMPID=e.EMPID AND d.DATE='$time_from')AS IN_TIME,
		(SELECT OUT_TIME FROM daily_emp_att d WHERE d.EMPID=e.EMPID AND d.DATE='$time_from')AS OUT_TIME,
		(SELECT START_TIME FROM shift_master sm WHERE sm.ID=e.SHIFT) AS START_TIME,
		(SELECT STOP_TIME FROM shift_master sm WHERE sm.ID=e.SHIFT) AS STOP_TIME

		FROM `employee` e 
		where e.WING_MASTER_ID=$WING_MASTER_ID and e.DESIG=$desig and e.STATUS=1");
		return $query->result_array();
	}
	
	
		public function getEmpDAta_new($time_from,$sec_inc_id,$role_id)
	{
			$query = $this->db->query("SELECT e.id, e.EMPID, e.EMP_FNAME, e.EMP_MNAME, e.EMP_LNAME , e.DESIG , e.WING_MASTER_ID,
		(SELECT ATT_STATUS FROM daily_emp_att d WHERE d.EMPID=e.EMPID AND d.DATE='$time_from')AS ATT_STATUS,
		(SELECT IN_TIME FROM daily_emp_att d WHERE d.EMPID=e.EMPID AND d.DATE='$time_from')AS IN_TIME,
		...
		where e.WING_MASTER_ID=$sec_inc_id and  e.ROLE_ID =$role_id and e.STATUS=1");
	...
	public function getEmployeeList($where,$gender='',$role='',$desig='')
	{
		$sql="SELECT employee.*,desig.DESIG,wing_master.NAME as wing_master_name,role_master.NAME as Role_name FROM `employee` INNER JOIN desig ON desig.Sno=employee.DESIG INNER JOIN wing_master ON employee.WING_MASTER_ID=wing_master.ID INNER JOIN role_master ON role_master.ID=employee.ROLE_ID WHERE $where and 1=1 order by employee.EMPID";
			
			if ($gender)
			{
					$sql .= " AND employee.SEX='".$gender."'";
			}
			if ($role)
			{
					$sql .= " AND role_master.ID='".$role."'";
			}
			if ($desig)
			{
					$sql .= " AND employee.DESIG='".$desig."'";
			}
