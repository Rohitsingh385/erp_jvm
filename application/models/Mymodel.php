...
class Mymodel extends CI_model{
	...
	public function classwise_strength_session($ses,$cls){
		$query = $this->db->query("SELECT CLASS,DISP_CLASS,
		(SELECT COUNT(*) FROM student WHERE student.CATEGORY='1' AND student.CLASS=st.CLASS AND Student_Status='ACTIVE')CAT1,
		(SELECT COUNT(*) FROM student WHERE student.CATEGORY='2' AND student.CLASS=st.CLASS AND Student_Status='ACTIVE')CAT2,
		(SELECT COUNT(*) FROM student WHERE student.CATEGORY='3' AND student.CLASS=st.CLASS AND Student_Status='ACTIVE')CAT3,
		(SELECT COUNT(*) FROM student WHERE student.CATEGORY='4' AND student.CLASS=st.CLASS AND Student_Status='ACTIVE')CAT4,
		(SELECT COUNT(*) FROM student WHERE student.CATEGORY='5' AND student.CLASS=st.CLASS AND Student_Status='ACTIVE')CAT5,
		(SELECT COUNT(*) FROM student WHERE student.CATEGORY='6' AND student.CLASS=st.CLASS AND Student_Status='ACTIVE')CAT6
		FROM student st WHERE Student_Status='ACTIVE' AND st.CLASS='$cls' AND st.ADM_NO LIKE '%$ses%' GROUP BY CLASS,DISP_CLASS;");
		return $query->result();
	}
	...
	public function half_year_stu_tbl_list($Class_No,$sec,$sortval,$exm_code,$subcode,$opt_code){
		if($opt_code != 2){
	...
	public function half_year_stu_tbl_list2($Class_No,$sec,$sortval,$exm_code,$subcode,$opt_code){
		if($opt_code != 2){
			$query = $this->db->query("SELECT st.`ADM_NO`,st.`FIRST_NM`,st.`ROLL_NO`,(SELECT M2 from marks where admno=st.ADM_NO AND Classes=st.CLASS and Sec=st.SEC and ExamC='$exm_code' and SCode='$subcode' and term='TERM-2')mrks2 FROM `student` as st where st.`CLASS`='$Class_No' AND st.`SEC`='$sec' AND st.`Student_Status`='ACTIVE' order by st.$sortval");
		}else{
			$query = $this->db->query("SELECT st.ADM_NO,st.FIRST_NM,st.ROLL_NO,(SELECT M2 from marks where admno=st.ADM_NO AND Classes=st.CLASS and Sec=st.SEC and ExamC='$exm_code' and SCode='$subcode' and term='TERM-2')mrks2 FROM `student` as st join studentsubject as ss on st.ADM_NO=ss.Adm_no where ss.SUBCODE='$subcode' AND ss.Class='$Class_No' AND ss.SEC='$sec' AND st.`CLASS`='$Class_No' AND st.`SEC`='$sec' AND st.`Student_Status`='ACTIVE' order by st.$sortval");
		}
		return $query->result();
	}
	
	public function half_year_stusub_tbl_list13($Class_No, $sec, $sortval, $exm_code, $subcode)
	{
		$query = $this->db->query("SELECT st.ADM_NO,st.FIRST_NM,st.ROLL_NO,ss.SUBCODE,
		(SELECT M2 from marks where admno=st.ADM_NO AND Classes=st.CLASS and Sec=st.SEC and ExamC='$exm_code' and SCode='$subcode' and term='TERM-1')mrks2 FROM `student` as st 
		join new_studentsubject_xi as ss on st.ADM_NO=ss.Adm_no 
		where ss.Class='$Class_No' AND ss.SEC='$sec' AND ss.SUBCODE='$subcode' AND st.`CLASS`='$Class_No' AND st.`SEC`='$sec' AND st.`Student_Status`='ACTIVE' order by st.$sortval");
		return $query->result();
	}
	...
	public function half_year_stusub_tbl_list($Class_No, $sec, $sortval, $exm_code, $subcode)
	{
		...
		where st.`CLASS`='$Class_No' AND st.`SEC`='$sec' AND st.`Student_Status`='ACTIVE' order by st.$sortval");
		return $query->result();
	}
	
	public function half_year_stusub_tbl_list_backup($Class_No,$sec,$sortval,$exm_code,$subcode){
		$query = $this->db->query("SELECT st.ADM_NO,st.FIRST_NM,st.ROLL_NO,(SELECT M2 from marks where admno=st.ADM_NO AND Classes=st.CLASS and Sec=st.SEC and ExamC='$exm_code' and SCode='$subcode' and term='TERM-1')mrks2 FROM `student` as st join studentsubject as ss on st.ADM_NO=ss.Adm_no where st.`CLASS`='$Class_No' AND st.`SEC`='$sec' AND st.`Student_Status`='ACTIVE' order by st.$sortval");
		return $query->result();
	}
	
	
	public function half_year_stusub_tbl_list_xii($Class_No,$sec,$exm_code,$subcode){
		$query = $this->db->query("SELECT st.ADM_NO,st.FIRST_NM,st.ROLL_NO,ss.Student_code,ss.STU_subcode,
		(SELECT M2 from marks where admno=st.ADM_NO AND Classes=st.CLASS and Sec=st.SEC and ExamC='$exm_code' and SCode='$subcode' and term='TERM-1')mrks2 
		FROM `student` as st 
		join studentsubject_xi as ss on st.ADM_NO=ss.Adm_no
		where ss.SUBCODE='$subcode' AND ss.Class='$Class_No' AND ss.GROUP='$sec' AND st.`CLASS`='$Class_No' AND st.`Student_Status`='ACTIVE' order by ss.STU_subcode;");
	...
	public function half_year_stu_tbl_list2_xii($Class_No,$sec,$exm_code,$subcode,$opt_code){
		if($opt_code != 2){
			$query = $this->db->query("SELECT st.`ADM_NO`,st.`FIRST_NM`,st.`ROLL_NO`,ss.STU_subcode,(SELECT M2 from marks where admno=st.ADM_NO AND Classes=st.CLASS and Sec=st.SEC and ExamC='$exm_code' and SCode='$subcode' and term='TERM-2')mrks2 FROM `student` as st where st.`CLASS`='$Class_No' AND st.`SEC`='$sec' AND st.`Student_Status`='ACTIVE' order by ss.STU_subcode");
		}else{
			$query = $this->db->query("SELECT st.ADM_NO,st.FIRST_NM,st.ROLL_NO,ss.STU_subcode,
			(SELECT M2 from marks where admno=st.ADM_NO AND Classes=st.CLASS and Sec=st.SEC and ExamC='$exm_code' and SCode='$subcode' and term='TERM-1')mrks2
			FROM `student` as st 
			join studentsubject_xi as ss on st.ADM_NO=ss.Adm_no
			where ss.SUBCODE='$subcode' AND ss.Class='$Class_No' AND ss.GROUP='$sec' AND st.`CLASS`='$Class_No'  AND st.`Student_Status`='ACTIVE' order by ss.STU_subcode");
	...
	{
		$query = $this->db->query("SELECT driver_master.*,
		bus_trip_master.Trip_Nm,bus_incharge_master.Incharge_nm,
		busnomaster.BusNo,bus_incharge_master.Incharge_ph_no FROM(((driver_master LEFT JOIN busnomaster ON 					driver_master.BusCode=busnomaster.BusCode) LEFT JOIN bus_incharge_master ON bus_incharge_master.Incharge_Id=driver_master.incharge_id) LEFT JOIN bus_trip_master ON bus_trip_master.Trip_ID=driver_master.trip_id) WHERE driver_master.Driver_ID='$id'");
		return $query->row();
		return $query->row();
	}
	...
	public function junior_rectno(){
		$query = $this->db->query("SELECT MAX(CAST(SUBSTR(TRIM(RECT_NO),3) AS UNSIGNED)+1) MAX_NUMBER FROM daycoll WHERE RECT_NO RLIKE 'JT'");
		return $query->result();
	}
	...
	public function getbusstoppagedetails($buscode,$tripid,$preferid){
		$query = $this->db->query("SELECT bus_route_master.BusCode,bus_route_master.Route_Id,stoppage.STOPPAGE,bus_route_master.STOPNO,bus_trip_master.Trip_Nm,bus_trip_master.trip_ID,bus_route_master.Prefer_ID FROM ((bus_route_master LEFT JOIN stoppage ON bus_route_master.STOPNO=stoppage.STOPNO) LEFT JOIN bus_trip_master ON bus_route_master.Trip_ID=bus_trip_master.Trip_ID) WHERE bus_route_master.BusCode LIKE '$buscode' AND bus_route_master.Trip_ID LIKE '$tripid' AND bus_route_master.Prefer_ID LIKE '$preferid'");
		return $query->result();
	}
	...
	function update_student($admno,$status)
	{
			
		$query = $this->db->query("UPDATE admitcard SET sts=".$status." WHERE AdmNo='".$admno."'");
		return true;

	}

	function admit_card_entrystatus_tbl($data){
		if($this->db->insert('admitcard_entrystatus',$data))
			return $this->db->insert_id();
		else
			return FALSE;

	}
	function check_class_section_status($class_no,$sec)
	{
		$sql="SELECT * FROM admitcard_entrystatus WHERE classno=? AND sec=? AND STATUS=1";
		$query = $this->db->query($sql,array($class_no,$sec));
	...
	function get_student_details($class_name,$sec_name)
	{
		
		$class_sec=$class_name.'-'.$sec_name;
		$sql="SELECT a.*,b.ROLL_NO FROM admitcard a
			INNER JOIN student b ON b.ADM_NO=a.AdmNo WHERE Class_Sec='".$class_sec."'";
	...
	function get_class_name($class_no)
	{
		$sql="SELECT CLASS_NM FROM classes WHERE Class_No=?";

			$query = $this->db->query($sql,array($class_no));
	        if ($this->db->affected_rows() > 0) 
	        {
	            return $query->row()->CLASS_NM;
	        } else {
	            return false;
	        }
	}
	function get_sec_name($sec_no)
	{
		$sql="SELECT SECTION_NAME FROM sections WHERE section_no=?";

			$query = $this->db->query($sql,array($sec_no));
	        if ($this->db->affected_rows() > 0) 
	        {
	...
	function get_student_status($admno,$class_sec){

		$sql="SELECT a.*,b.* FROM admitcard a
				INNER JOIN student b ON b.ADM_NO=a.AdmNo
				WHERE a.AdmNo=? AND a.Class_Sec=?";

			$query = $this->db->query($sql,array($admno,$class_sec));
	        if ($this->db->affected_rows() > 0) 
	        {
	            return $query->row();
	        } else {
	...
	public function topper_10($exam_type,$subj_name,$class_name,$term){
		$query = $this->db->query("SELECT marks.M3 , marks.admno ,
		student.FIRST_NM , student.MIDDLE_NM ,student.DISP_CLASS,student.DISP_SEC,student.ROLL_NO FROM `marks`
		INNER JOIN student 
		ON marks.admno=student.ADM_NO
		WHERE ExamC='$exam_type' AND SCode='$subj_name' AND Classes='$class_name' AND TERM='$term' ORDER BY `marks`.`M3` DESC LIMIT 10");
		return $query->result();
	}


	function get_admit_card($admno){
	$sql="SELECT `Date1` AS edate,`Day1` AS eday,Subj1 AS esub FROM admitcard WHERE AdmNo=?
AND DATE1!='NA' AND DATE1!=''
union
SELECT `Date2` AS edate,`Day2` AS eday,Subj2 AS esub FROM admitcard WHERE AdmNo=?
AND DATE2!='NA' AND DATE2!=''
union
SELECT `Date3` AS edate,`Day3` AS eday,Subj3 AS esub FROM admitcard WHERE AdmNo=?
AND DATE3!='NA' AND DATE3!=''
union
SELECT `Date4` AS edate,`Day4` AS eday,Subj4 AS esub FROM admitcard WHERE AdmNo=?
AND DATE4!='NA' AND DATE4!=''
union
SELECT `Date5` AS edate,`Day5` AS eday,Subj5 AS esub FROM admitcard WHERE AdmNo=?
AND DATE5!='NA' AND DATE5!=''
union
SELECT `Date6` AS edate,`Day6` AS eday,Subj6 AS esub FROM admitcard WHERE AdmNo=?
AND DATE6!='NA' AND DATE6!=''
union
SELECT `Date7` AS edate,`Day7` AS eday,Subj7 AS esub FROM admitcard WHERE AdmNo=?
AND DATE7!='NA' AND DATE7!=''
union
SELECT `Date8` AS edate,`Day8` AS eday,Subj8 AS esub FROM admitcard WHERE AdmNo=?
AND DATE8!='NA' AND DATE8!=''
union
SELECT `Date9` AS edate,`Day9` AS eday ,Subj9 AS esub FROM admitcard WHERE AdmNo=?
AND DATE9!='NA' AND DATE9!=''
union
SELECT `Date10` AS edate,`Day10` AS eday,Subj10 AS esub FROM admitcard WHERE AdmNo=?
AND DATE10!='NA' AND DATE10!=''";


$query = $this->db->query($sql,array($admno,$admno,$admno,$admno,$admno,$admno,$admno,$admno,$admno,$admno));
	        if ($this->db->affected_rows() > 0) 
	        {
	            return $query->result();
	        } else {
