...
class Alam extends CI_model{
	...
	public function co_scholastic_grade_data_NEW($classs,$sec,$trm){
        $query = $this->db->query("SELECT  DISTINCT(stu.ADM_NO),stu.FIRST_NM,stu.ROLL_NO,(select Grade from co_scholastic_grade where Adm_no=stu.ADM_NO and SkillCode = 1 and Term = $trm)skill1,(select Grade from co_scholastic_grade where Adm_no=stu.ADM_NO and SkillCode = 2 and Term = $trm)skill2,(select Grade from co_scholastic_grade where Adm_no=stu.ADM_NO and SkillCode = 3 and Term = $trm)skill3,(select Grade from co_scholastic_grade where Adm_no=stu.ADM_NO and SkillCode = 4 and Term = $trm)skill4,(select Grade from co_scholastic_grade where Adm_no=stu.ADM_NO and SkillCode = 5 and Term = $trm)skill5 from `student` as stu where stu.student_status='ACTIVE' and stu.CLASS='$classs' AND stu.SEC='$sec' order by stu.ROLL_NO");
		return $query->result();
	}
	public function persnality_grade_data_NEW($classs,$sec,$trm){
        $query = $this->db->query("SELECT  DISTINCT(stu.ADM_NO),stu.FIRST_NM,stu.ROLL_NO,
		(select Grade from personalityasses_grade where Adm_no=stu.ADM_NO and perCode = 1 and Term = $trm)skill1,
		(select Grade from personalityasses_grade where Adm_no=stu.ADM_NO and perCode = 2 and Term = $trm)skill2,
		(select Grade from personalityasses_grade where Adm_no=stu.ADM_NO and perCode = 3 and Term = $trm)skill3,
		(select Grade from personalityasses_grade where Adm_no=stu.ADM_NO and perCode = 4 and Term = $trm)skill4,
		(select Grade from personalityasses_grade where Adm_no=stu.ADM_NO and perCode = 5 and Term = $trm)skill5,
		(select Grade from personalityasses_grade where Adm_no=stu.ADM_NO and perCode = 6 and Term = $trm)skill6
		from `student` as stu where stu.student_status='ACTIVE' and stu.CLASS='$classs' AND stu.SEC='$sec' order by stu.ROLL_NO");
		return $query->result();
	}
	public function del_grd_tbl1($class,$sec,$trm){
		$this->db->query("delete from personalityasses_grade where Class='$class' AND Sec='$sec' AND Term='$trm'");
		//$this->db->query("delete from discipline_grades where Class='$class' AND Sec='$sec' AND Term='$trm'");
		return true;
	}
	...
	public function report_card_student_detail_xi($trm,$classs,$sec){
		$query = $this->db->query("SELECT stu.ADM_NO,stu.int_report_card_status,stu.t1_report_card_status,stu.t2_report_card_status,stu.DISP_CLASS,stu.DISP_SEC,stu.ROLL_NO,stu.FIRST_NM,stu.MIDDLE_NM,stu.TITLE_NM,stu.FATHER_NM,stu.MOTHER_NM,stu.BIRTH_DT
	
		 from student as stu where stu.CLASS = '$classs' AND stu.SEC = '$sec' AND Student_Status = 'ACTIVE' order by stu.ROLL_NO");
		return $query->result();
	}
	public function report_card_student_detail($trm,$classs,$sec){
		$query = $this->db->query("SELECT stu.ADM_NO,stu.int_report_card_status,stu.t1_report_card_status,stu.t2_report_card_status,stu.DISP_CLASS,stu.DISP_SEC,stu.ROLL_NO,stu.FIRST_NM,stu.MIDDLE_NM,stu.TITLE_NM,stu.FATHER_NM,stu.MOTHER_NM,stu.BIRTH_DT,stu.WORK_DAYS from student as stu where stu.CLASS = '$classs' AND stu.SEC = '$sec' AND Student_Status = 'ACTIVE' order by stu.ROLL_NO");
		return $query->result();
	}
	
	public function report_card_student_detail_int($trm,$classs,$sec){
	...
	public function annual_report_card_student_detail_new($classs,$sec){
		$query = $this->db->query("SELECT stu.ADM_NO,stu.DISP_CLASS,stu.DISP_SEC,stu.ROLL_NO,stu.FIRST_NM,stu.MIDDLE_NM,stu.TITLE_NM,stu.FATHER_NM,stu.MOTHER_NM,stu.BIRTH_DT from student as stu where stu.CLASS = '$classs' AND stu.SEC = '$sec' AND Student_Status = 'ACTIVE' order by stu.ROLL_NO");
		return $query->result();
	}
	...
	public function getClassWiseSubject($term,$class,$section)
	{
		if($class==9 || $class==10)
		{
			$query = $this->db->query("SELECT DISTINCT subject_code,subj_nm,opt_code,sorting_no,applicable_exam,ifnull((SELECT ExamMode FROM classes WHERE Class_No=cswsa.Class_No),1)exam_mode,ifnull((SELECT PT_TYPE FROM classes WHERE Class_No=cswsa.Class_No),1)pt_type FROM `class_section_wise_subject_allocation` cswsa WHERE Class_No='$class' AND applicable_exam=1 AND subject_code!=2 ORDER BY cswsa.sorting_no asc");
	...
	public function get_consolidated_report($exam,$admno,$sub)
	{
		$sql="SELECT * FROM marks WHERE EXAMC=? AND ADMNO=? AND SCODE=?";
		$query=$this->db->query($sql,array($exam,$admno,$sub));
		return $query->row()->M3;
	}

	public function get_grade_consolidated($marks)
	{
		$sql="SELECT Grade FROM grademaster WHERE ORange>=? AND CRange<=?";
		$query=$this->db->query($sql,array($marks,$marks));
